...
def set_option_value(multiworld: MultiWorld, player: int, name: str, value) -> None:
...
def before_create_regions(world: World, multiworld: MultiWorld, player: int):
    battle_seed = get_option_value(multiworld, player, "battle_seed")
    if battle_seed < 0:
        seed = random.randint(1, 999999999999)
        set_option_value(multiworld, player, "battle_seed", seed)
...
def after_create_regions(world: World, multiworld: MultiWorld, player: int):
    ...
    if not getattr(multiworld, "generation_is_fake", False):
        ...
        battleLocations = list(world.location_name_groups["Battle Locations"])
        battleLocationsToKeep = rng.sample(battleLocations, 40)
        locationNamesToRemove.extend([x for x in battleLocations if x not in battleLocationsToKeep])
