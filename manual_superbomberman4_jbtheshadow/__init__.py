...
class ManualWorld(World):
                ...
                for cat, count in configs.items():
                    ...
                    if isinstance(cat, ItemClassification):
                        ...
                        try:
                            ...
                            raise Exception(
                                f"Item override '{cat}' for {name} improperly defined\n\n{type(ex).__name__}:{ex}"
                            )
    ...
    def get_item_counts(
        self, player: Optional[int] = None, pool: list[Item] | None | bool = None, only_progression: bool = False
    ) -> Counter[str]:
        ...
        if isinstance(pool, bool):
            Utils.deprecate(
                "the 'reset' argument of get_item_counts has been deprecated to increase the stability of item counts.\
