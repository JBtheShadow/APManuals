...
if len(victory_names) > 1:
    if manual_options.get("goal"):
        logging.warning(
            "Existing Goal option found created via Hooks, it will be overwritten by Manual's generated Goal option.\nIf you want to support old yaml you will need to add alias in after_options_defined"
        )
    ...
    if manual_options.get(option_name):
        ...
        if issubclass(original_option, Toggle):
            ...
            if option.get("values"):
                raise Exception(
                    f"You cannot modify the values of the '{option_name}' option since they cannot have their value changed by Option.json"
                )

            if option.get("aliases"):
                for alias, value in option["aliases"].items():
                    original_option.aliases[alias] = value
                original_option.options.update(
                    original_option.aliases
                )  # for an alias to be valid it must also be in options
            ...
            if option.get("values"):  # let user add named values
                ...
                args["special_range_names"]["default"] = option.get(
                    "default", args["special_range_names"].get("default", args["default"])
                )
                args["range_start"] = original_option.range_start
                args["range_end"] = original_option.range_end
                args["special_range_names"] = {
                    **args["special_range_names"],
                    **{l.lower(): v for l, v in option["values"].items()},
                }
        ...
        logging.debug(
            f"Manual: Option.json just tried to modify the option '{option_name}' but it doesn't currently exists"
        )
...
for option_name, option in option_table.get("user", {}).items():
    ...
    if manual_options.get(option_name):
        logging.warning(
            f"Manual: An option with the name '{option_name}' cannot be added since it already exists in Manual Core Options. \nTo modify an existing option move it to the 'core' section of Option.json"
        )
        ...
        if option_type not in supported_option_types:
            raise Exception(
                f'Option {option_name} in options.json has an invalid type of "{option["type"]}".\nIt must be one of the folowing: {supported_option_types}'
            )
...
def make_options_group() -> list[OptionGroup]:
    ...
    if manual_option_groups:
        if "Item & Location Options" in manual_option_groups.keys():
            base_item_loc_group = manual_option_groups.pop(
                "Item & Location Options"
            )  # Put the custom options before the base AP options
