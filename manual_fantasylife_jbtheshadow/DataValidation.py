...
class DataValidation:
            ...
            if isinstance(location["requires"], str):
                ...
                for item in re.findall(r"\|[^|]+\|", location["requires"]):
                    if item.lower() == "or" or item.lower() == "and" or item == ")" or item == "(":
                        ...
                        item_exists = (
                            len([item["name"] for item in DataValidation.item_table if item["name"] == item_name]) > 0
                        )

                        if not item_exists:
                            raise ValidationError(
                                "Item %s is required by location %s but is misspelled or does not exist."
                ...
                for item in location["requires"]:
                    ...
                    if (isinstance(item, dict) and "or" in item and isinstance(item["or"], list)) or (
                        ...
                        for or_item in or_items:
                            ...
                            item_exists = (
                                len(
                                    [item["name"] for item in DataValidation.item_table if item["name"] == or_item_name]
                                )
                                > 0
                            )
            ...
            if isinstance(region["requires"], str):
                ...
                for item in re.findall(r"\|[^|]+\|", region["requires"]):
                    if item.lower() == "or" or item.lower() == "and" or item == ")" or item == "(":
                        ...
                        if not item_exists:
                            raise ValidationError(
                                "Item %s is required by region %s but is misspelled or does not exist."
    ...
    def checkRegionNamesInLocations():
        for location in DataValidation.location_table:
            ...
            if not region_exists:
                raise ValidationError(
                    "Region %s is set for location %s, but the region is misspelled or does not exist."
            ...
            for location in DataValidation.location_table:
                ...
                if isinstance(location_requires, str):
                    if "|{}|".format(item["name"]) in location_requires:
                        raise ValidationError(
                            "Item %s is required by location %s, but the item is not marked as progression."
            ...
            for region_name in DataValidation.region_table:
                ...
                if isinstance(region_requires, str):
                    if "|{}|".format(item["name"]) in region_requires:
                        raise ValidationError(
                            "Item %s is required by region %s, but the item is not marked as progression."
        ...
        if values_requested:
            ...
            for value, count in values_requested.items():
                if values_available.get(value, 0) < count:
                    errors.append(
                        f"   '{value}': {values_available.get(value, 0)} out of the {count} {value} worth of progression items required can be found."
                    )
            if errors:
                raise ValidationError(
                    "There are not enough progression items for the following values: \n" + "\n".join(errors)
                )

    @staticmethod
    def preFillCheckIfEnoughItemsForValue(world: World, multiworld: MultiWorld):
        from .Helpers import get_items_for_player, get_items_with_value
            ...
            for location in region.locations:
                ...
                if "requires" in manualLocation and manualLocation["requires"]:
                    DataValidation._checkLocationRequiresForItemValueWithRegex(
                        values_requested, manualLocation["requires"]
                    )
        ...
        if values_requested:
            ...
            existing_items = [
                item
                for item in get_items_for_player(multiworld, player, True)
            ...
            for value, val_count in values_requested.items():
                ...
                if found_count < val_count:
                    errors.append(
                        f"   '{value}': {found_count} out of the {val_count} {value} worth of progression items required can be found."
                    )
            if errors:
                raise ValidationError(
                    "There are not enough progression items for the following value(s): \n" + "\n".join(errors)
                )
    ...
    def checkRegionsConnectingToOtherRegions():
        for region_name in DataValidation.region_table:
            ...
            for connecting_region in region["connects_to"]:
                ...
                if not region_exists:
                    raise ValidationError(
                        "Region %s connects to a region %s, which is misspelled or does not exist."
                        % (region_name, connecting_region)
                    )
    ...
    def checkStartingItemsForValidItemsAndCategories():
        ...
        for starting_block in starting_items:
            if "items" in starting_block and "item_categories" in starting_block:
                raise ValidationError(
                    "One of your starting item definitions has both 'items' and 'item_categories' defined, but only one will be applied."
                )

            if "items" in starting_block:
                for item_name in starting_block["items"]:
                    if item_name not in [item["name"] for item in DataValidation.item_table]:
                        raise ValidationError(
                            "Item %s is set as a starting item, but is misspelled or is not defined." % (item_name)
                        )
                    ...
                    if (
                        len(
                            [
                                item
                                for item in DataValidation.item_table
                                if "category" in item and category_name in item["category"]
                            ]
                        )
                        == 0
                    ):
                        raise ValidationError(
                            "Item category %s is set as a starting item category, but is misspelled or is not defined on any items."
                            % (category_name)
                        )
    ...
    def checkStartingItemsForBadSyntax():
        ...
        for starting_block in starting_items:
            ...
            valid_keys = [
                ...
                "_comment",
                "yaml_option",
            ]  # _comment is provided by schema
            invalid_keys = [f'"{key}"' for key in starting_block.keys() if key not in valid_keys]

            if len(invalid_keys) > 0:
                raise ValidationError(
                    "One of your starting item definitions is invalid and may have unexpected results.\n   The invalid starting item definition specifies the following incorrect keys: {}".format(
                        ", ".join(invalid_keys)
                    )
                )
    ...
    def checkPlacedItemsForValidItems():
        for location in DataValidation.location_table:
            ...
            for item_name in place_item:
                if item_name not in [item["name"] for item in DataValidation.item_table]:
                    raise ValidationError(
                        "Item %s is placed (using place_item) on a location, but is misspelled or is not defined."
                        % (item_name)
                    )
        ...
        for location in DataValidation.location_table:
            ...
            for category_name in place_item_category:
                if (
                    ...
                    raise ValidationError(
                        "Item category %s is placed (using place_item_category) on a location, but is misspelled or is not defined."
                        % (category_name)
                    )
    ...
    def checkForGameBeingInvalidJSON():
        if len(DataValidation.game_table) == 0:
            raise ValidationError(
                "No settings were found in your game.json. This likely indicates that your JSON is incorrectly formatted. Use https://jsonlint.com/ to validate your JSON files."
            )
    ...
    def checkForItemsBeingInvalidJSON():
        if len(DataValidation.item_table) == 0:
            raise ValidationError(
                "No items were found in your items.json. This likely indicates that your JSON is incorrectly formatted. Use https://jsonlint.com/ to validate your JSON files."
            )
    ...
    def checkForLocationsBeingInvalidJSON():
        if len(DataValidation.location_table) == 0:
            raise ValidationError(
                "No locations were found in your locations.json. This likely indicates that your JSON is incorrectly formatted. Use https://jsonlint.com/ to validate your JSON files."
            )

    @staticmethod
    def checkForNonStartingRegionsThatAreUnreachable():
        using_starting_regions = (
            len(
                [
                    region
                    for region in DataValidation.region_table
                    if "starting" in DataValidation.region_table[region]
                    and not DataValidation.region_table[region]["starting"]
                ]
            )
            > 0
        )
        ...
        nonstarting_regions = [
            region
            for region in DataValidation.region_table
            if "starting" in DataValidation.region_table[region] and not DataValidation.region_table[region]["starting"]
        ]

        for nonstarter in nonstarting_regions:
            regions_that_connect_to = [
                region
                for region in DataValidation.region_table
                if "connects_to" in DataValidation.region_table[region]
                and nonstarter in DataValidation.region_table[region]["connects_to"]
            ]

            if len(regions_that_connect_to) == 0:
                raise ValidationError(
                    "The region '%s' is set as a non-starting region, but has no regions that connect to it. It will be inaccessible."
                    % nonstarter
                )
...
def runPreFillDataValidation(world: World, multiworld: MultiWorld):
    ...
    try:
        DataValidation.preFillCheckIfEnoughItemsForValue(world, multiworld)
...
def runGenerationDataValidation() -> None:
    ...
    try:
        DataValidation.checkIfEnoughItemsForValue()
    except ValidationError as e:
        validation_errors.append(e)
